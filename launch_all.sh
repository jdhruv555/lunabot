...
screen -S mars -X quit >/dev/null 2>&1 || true
screen -S slam -X quit >/dev/null 2>&1 || true
screen -S nav2 -X quit >/dev/null 2>&1 || true

screen -dmS mars bash -lc 'export AMENT_TRACE_SETUP_FILES=0; source /opt/ros/humble/setup.bash; [ -f /ws/install/setup.bash ] && source /ws/install/setup.bash || true; ros2 launch lunabot_sim spawn_in_world.launch.py world:=/ws/src/lunabot_sim/worlds/mars.world'
screen -dmS slam bash -lc 'export AMENT_TRACE_SETUP_FILES=0; source /opt/ros/humble/setup.bash; [ -f /ws/install/setup.bash ] && source /ws/install/setup.bash || true; ros2 launch lunabot_mapping mapping.launch.py use_sim_time:=true'
